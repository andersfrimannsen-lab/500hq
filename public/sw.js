const CACHE_NAME = 'hopeful-quotes-v17';
const STATIC_ASSETS = [
    '/',
    '/index.html',
    '/index.css',
...
function scheduleDailyNotification() {
  ...
  dailyNotificationTimer = setTimeout(() => {
    ...
    self.registration.showNotification('Hopeful Quotes', {
      ...
      tag: 'daily-quote-notification',
      renotify: true,
      data: { action: 'focus_app' } // No URL, just focus action
    });
...
// Pure native app notification click handler - NO URL navigation
self.addEventListener('notificationclick', (event) => {
  ...
  const focusApp = async () => {
    try {
      console.log('Attempting to focus existing PWA client...');

      // Get all client windows - focus on PWA clients only
      ...
      if (clientList.length > 0) {
        // Find the best client to focus
        let clientToFocus = null;
        
        // Priority 1: Find a focused client
        ...
        // Priority 2: Find a client with matching origin (PWA)
        if (!clientToFocus) {
          for (const client of clientList) {
        ...
        // Priority 3: Use any available client
        if (!clientToFocus) {
          clientToFocus = clientList[0];
        }
        
        console.log('Focusing client:', clientToFocus.url);
        
        // Simply focus the existing client - NO navigation
        await clientToFocus.focus();
        
        // Send a message to the client to handle any app-specific actions
      ...
      // Only if no client exists, open the app (this should be rare for installed PWAs)
      console.log('No existing clients found, opening app...');
      if (clients.openWindow) {
        // Open without specific URL to avoid web navigation
        const newClient = await clients.openWindow('/');
        console.log('Opened new client:', newClient ? 'success' : 'failed');
        return newClient;
      }
    } catch (error) {
      console.error('Error focusing app:', error);
      // Last resort: try to open the app
      if (clients.openWindow) {
        try {
